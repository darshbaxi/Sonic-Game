...
func init() {
	port := os.Getenv("PORT")
	if port == "" {
		port = "8007" // Default port if not specified
	}
	addr = ":" + port // Updated to work on Render
}
...
func main() {
	...
	indexPageContent = bytes.Replace(indexPageContentRaw, []byte("%APP_ENV%"), []byte(*appEnv), 1)
	indexPageContent = bytes.Replace(indexPageContent, []byte("%APP_VERSION%"), bytes.TrimSpace(version), 2)
